...
parametric_shapes::createQuad(unsigned int width, unsigned int height)
{
	...
	glGenVertexArrays(1, /*! \todo */ &data.vao /*nullptr*/);

	// To be able to store information, the Vertex Array has to be bound
	// first.
	glBindVertexArray(/*! \todo bind the previously generated Vertex Array */ data.vao /*0u*/);
	...
	// `data.bo`.
	glGenBuffers(1, /*! \todo */ &data.bo /*nullptr*/);
	...
	glBindBuffer(GL_ARRAY_BUFFER, /*! \todo bind the previously generated Buffer */ data.bo /*0u*/);
	...
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::vertices),
	                      /*! \todo how many components do our vertices have? */ 3 /*0*/,
	...
	glGenBuffers(1, /*! \todo */ &data.ibo /*nullptr*/);

	// We still want a 1D-array, but this time it should be a 1D-array of
	// elements, aka. indices!
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, /*! \todo bind the previously generated Buffer */ data.ibo /*0u*/);

	glBufferData(GL_ELEMENT_ARRAY_BUFFER, /*! \todo how many bytes should the buffer contain? */ static_cast<GLsizeiptr>(indices.size() * sizeof(glm::uvec3)) /*0u*/,
	             /* where is the data stored on the CPU? */ reinterpret_cast<GLvoid const*>(indices.data()) /*indices.data()*/,
	             /* inform OpenGL that the data is modified once, but used often */GL_STATIC_DRAW);

	data.indices_nb = /*! \todo how many indices do we have? */ indices.size() * 3u /*0u*/;
...
bonobo::mesh_data
parametric_shapes::createQuad(unsigned int res_width, unsigned int res_depth, unsigned int width, unsigned int depth)
{
	auto const vertices_nb = res_width * res_depth;
	...
	for (unsigned int i = 0; i < res_depth; i++) {
		...
		for (unsigned int j = 0; j < res_width; j++) {
			...
			texcoords[index] = glm::vec3(static_cast<float>(j) / (static_cast<float>(res_width) - 1.0f),
				static_cast<float>(i) / (static_cast<float>(res_depth) - 1.0f),
				0.0f);
	...
	index = 0u;
	for (unsigned int i = 0u; i < res_depth - 1u; ++i) {
